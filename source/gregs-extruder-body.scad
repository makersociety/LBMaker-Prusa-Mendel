...
idler_fulcrum=idler_axis-[2,idler_fulcrum_offset,0];
	...
	difference ()
	{
		union()
		{
			...
			translate(idler_fulcrum)
			{
				...
				cube([idler_hinge_r*2,idler_hinge_r*2,idler_short_side-2*idler_hinge_width-.5],
					center=true);
				rotate(-30)
				{
...
%			rotate([0,180,0])
			...
			for (mount=[0:1])
			{
				...
				rotate([-90,0,0])
				rotate(360/16)
				cylinder(r=m4_diameter/2+.125,h=base_thickness+2,$fn=8);	
	
				translate([-filament_feed_hole_offset+25*((mount<1)?1:-1),
					-motor_mount_translation[1]+base_thickness/2,
					wade_block_depth/2])
				rotate([-90,0,0])
				cylinder(r=m4_nut_diameter/2+.5,h=base_thickness,$fn=6);	
			}
...
%		translate([0,0,-8])
	...
	for (idle=[-1,1])
	{
		...
		rotate([0,90,0])
		{
			...
			{
				translate([0,0,-1])
				cylinder(r=m4_diameter/2+.8,h=wade_block_depth+6,$fn=6);	
				translate([0,0,wade_block_width-idler_nut_trap_depth])
				cylinder(r=(m4_nut_diameter/2),h=idler_nut_thickness,$fn=6);	
			}
			translate([0,(10/2)-3.1,(wade_block_width-idler_nut_trap_depth+idler_nut_thickness/2)+1.25])
			cube([(m4_nut_diameter*cos(30))+.5,12,idler_nut_thickness+1],center=true);
		}
	}
}
...
#			cube([guide_height+1,guide_length,8],center=true);
		...
		translate(idler_fulcrum)
		{
			#cylinder(h=idler_short_side-2*idler_hinge_width,
				r=idler_hinge_r+0.75,center=true,$fn=60);
			rotate(-30)
		...
		translate(idler_axis)
		{
			difference()
			{
				...
				// Bearing seat 
				translate([0,0,9/2])
				cylinder(r1=12/2,r2=18/2,h=2);
			}
			cylinder(h=idler_short_side,r=m8_diameter/2,
				center=true,$fn=20);
		}
		...
		translate(idler_fulcrum)
		rotate(360/12)
		#cylinder(h=idler_short_side+2,r=m3_diameter/2,center=true,$fn=8);

		//Nut trap for fulcrum screw.
		translate(idler_fulcrum+[0,0,idler_short_side/2-idler_hinge_width-1])
		rotate(360/16)
		cylinder(h=3,r=m3_nut_diameter/2+.25,$fn=6);
		...
		translate(idler_axis+[2-idler_height,0,0])
		{
			...
			{
				// Holes in Idles
				cylinder(r=idler_mounting_hole_diameter/2+.75,h=idler_height+2,$fn=16);
				translate([0,idler_mounting_hole_elongation,0])
				cylinder(r=idler_mounting_hole_diameter/2+.75,h=idler_height+2,$fn=16);
				translate([-idler_mounting_hole_diameter/2,0,0])
				cube([idler_mounting_hole_diameter+.75,idler_mounting_hole_elongation,
					idler_height+2]);
			}
...
{
	extruder_recess_d=16.75; 
	extruder_recess_h=5.4;
