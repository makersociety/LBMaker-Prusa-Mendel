...
difference(){
	union(){
		...
		translate([17, 17.5, 5]) rotate([90, 0, 0]) cylinder(h =5, r = 5.75, $fn = 6);
	}
	...
	translate([outer_diameter, outer_diameter, 0]) #cylinder(h =20, r = m8_diameter/2 + .125, $fn = 18);

	//Securing hole
	translate([17, 17, 5]) rotate([90, 0, 0]) #cylinder(h =20, r = m3_diameter/2, $fn = 10);
	translate([17, 19.5, 5]) rotate([90, 0, 0]) #cylinder(h =5, r = m3_nut_diameter_horizontal/2 + .5, $fn = 6);

	translate([17, 17, 5]) rotate([90, 0, 0]) #cylinder(h =20, r = m3_diameter/2, $fn = 10);
	translate([-11, 17, 5]) rotate([90, 0, 0]) #cylinder(h =20, r = m3_diameter/2, $fn = 10);
	translate([-(4+screw_hole_spacing)-7, 17, 5]) rotate([90, 0, 0]) #cylinder(h =20, r = m3_diameter/2, $fn = 10);
}
}
endstop();
