...
//		belt_support_offset=25+17.5;
			...
			for (i=[-1,1])
			translate([-25-13.5-2,i*(lm8uu_holder_length/2+belt_clamp_width/2)])
			rotate(90*(i+1)+180) 
			#belt_clamp_socket ();
		}
		...
		rotate(hole*22)
		translate([0,25,-1])
		#cylinder(r=m4_diameter/2+.25,h=lm8uu_support_thickness*2+2,$fs=1);
...
module belt_clamp_holes()
{
	translate([0,0,belt_clamp_height/2])
	{
		#for(i=[-1,1])
		translate([i*belt_clamp_hole_separation/2,0,0])
		cylinder(r=(m3_diameter/2)+.15,h=belt_clamp_height+2,center=true,$fn=8);
		...
		cylinder(r=m3_nut_diameter/2+.2,h=3.4,center=true,$fn=6);
	}
}
	...
	difference()
	{
		...
		#for(i=[-1,1])
		{
			translate([i*fan_hole_separation/2,0,fan_hole_height])
			{
				...
				cube([(m3_nut_diameter+.25)*cos(30),fan_trap_width+.5,fan_hole_height+1.2],center=true);
			}
		}
